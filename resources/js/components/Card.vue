<template>
  ...
  >
  <h1 class=" text-xl">{{ card.rollNo }}</h1>
  
    <h3 class="cardTitle">{{ card.name }}</h3>
    <div v-if="interactPosition.x>0" id="right-sign" :style="{opacity: interactPosition.x/100}">
     </div>
     <div v-else id="wrong-sign" :style="{opacity: -interactPosition.x/100}">
     </div>
  </div>
</template>
    ...
    playCard(interaction) {
      ...
      switch (interaction) {
        case ACCEPT_CARD:
          ...
          this.$emit(ACCEPT_CARD,this.card.name);
          break;
        case REJECT_CARD:
          this.interactSetPosition({
            x: -interactOutOfSightXCoordinate,
          ...
          this.$emit(REJECT_CARD,this.card.name);
          break;
        case SKIP_CARD:
          this.interactSetPosition({
...
#wrong-sign{
  @extend %sign;
background: url("https://s3-us-west-2.amazonaws.com/s.cdpn.io/617753/icon-reject.svg") center no-repeat;
  background-size:contain ;
}

#right-sign{
  @extend %sign;
background: url("https://s3-us-west-2.amazonaws.com/s.cdpn.io/617753/icon-approve.svg") center no-repeat;
  background-size:contain ;
}
